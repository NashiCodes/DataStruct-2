...
void LeftistHeap<T>::Insert(T key)
{
    ...
    LeftistHeapNode<T> *node = new LeftistHeapNode<T>(key);
...
T LeftistHeap<T>::RemoveMax()
{
    ...
    LeftistHeapNode<T> *temp = this->root;
    //   2. Realizar o merge dos filhos da raiz
    this->root = Auxiliary_Merge(this->root->left, this->root->right);
